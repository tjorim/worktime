...
const finishOnboardingSetup = async (user: ReturnType<typeof userEvent.setup>) => {
  if (screen.queryByRole("heading", { name: /Set Up Time Off/i })) {
    await user.click(screen.getByRole("button", { name: /Continue/i }));
  }
...
describe("WelcomeWizard", () => {
  ...
  describe("Time Off Setup Step", () => {
    ...
    it("should handle zero amount as valid (disables vacation tracking)", async () => {
      ...
      // Complete the remaining setup steps to trigger onHide
  ...
  describe("Integration tests", () => {
    ...
    it("should save vacation allowance when browsing all teams without selecting one", async () => {
      ...
      expect(saved.settings).toHaveProperty("enableGantt");
      expect(typeof saved.settings?.enableGantt).toBe("boolean");
      expect(saved.settings).toHaveProperty("enableCrossBorderTracking");
      expect(typeof saved.settings?.enableCrossBorderTracking).toBe("boolean");
